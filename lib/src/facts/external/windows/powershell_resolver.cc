...
namespace facter { namespace facts { namespace external {
    ...
    void powershell_resolver::resolve(string const& file, collection& facts) const
    {
        ...
        try
        {
            ...
            auto const ps_opts = vector<string> {
                "-NoProfile",
                "-NonInteractive",
                "-NoLogo",
                "-ExecutionPolicy",
            ...
            auto result = execute(pwrshell, ps_opts, 0, {
            ...
            if (!resolved) {
                ...
                lth::each_line(result.output, [&facts](string const& line) {
                    auto pos = line.find('=');
                    if (pos == string::npos) {
